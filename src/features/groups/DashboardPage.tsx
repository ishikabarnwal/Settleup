...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-12">
      <section className="relative overflow-hidden rounded-panel bg-ink px-6 py-8 text-white shadow-raised sm:px-12 sm:py-12">

        <div className="relative flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm text-white/70">Hi, {user.name.split(' ')[0]}</p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">Your groups</h1>
          </div>

          {groups.data && groups.data.length > 0 && (
            <dl className="grid grid-cols-2 gap-4 sm:flex" aria-busy={!allLoaded}>
...
function Summary({ label, value, tone }: { label: string; value: string; tone: string }) {
  return (
    <div className="rounded-card bg-white/6 px-6 py-4 ring-1 ring-white/10 sm:min-w-44">
      <dt className="text-xs text-white/65">{label}</dt>
      <dd className={clsx('mt-1 text-lg font-semibold tabular-nums', tone)}>{value}</dd>
    </div>
  )
}
...
function GroupCard({ group, myNet }: { group: GroupSummary; myNet: number | undefined }) {
  return (
    <Link
      to={`/groups/${group.id}`}
      className="group flex h-full flex-col rounded-card bg-surface p-6 shadow-card transition hover:-translate-y-1 hover:shadow-raised"
    >
      <div className="flex items-start justify-between gap-4">
        <h3 className="text-base font-semibold text-ink">{group.name}</h3>
        <ArrowRight className="mt-0.5 size-4 shrink-0 text-stone-400 transition group-hover:translate-x-1 group-hover:text-plum" />
      </div>
      {group.description ? (
        <p className="mt-2 line-clamp-2 text-sm text-stone-500">{group.description}</p>
      ) : (
...
function GroupGridSkeleton() {
  return (
    <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3" aria-label="Loading groups" aria-busy>
      {[0, 1, 2].map((i) => (
        <li key={i} className="h-44 animate-pulse rounded-card bg-surface p-6 shadow-card">
          <div className="h-4 w-2/5 rounded bg-stone-200" />
          <div className="mt-4 h-3 w-4/5 rounded bg-stone-100" />
          <div className="mt-16 h-3 w-1/3 rounded bg-stone-100" />
        </li>
      ))}
    </ul>
  )
